...
namespace GLCore::Utils {
    ...
    Model::Model(std::string const& path, const glm::vec3& position, const glm::vec3& rotation, const glm::vec3& scale)
        : Model(path)
    {
        m_Position = position;
		m_Rotation = rotation;
    ...
    std::vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
    {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {
                if (std::strcmp(m_LoadedTextures[j].m_FilePath.data(), str.C_Str()) == 0)
                {
            ...
            {   // if texture hasn't been loaded already, load it
                Texture texture(m_Directory + str.C_Str(), typeName);
